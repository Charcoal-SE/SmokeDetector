    ...
    def _init_websocket(self):
            ...
            if "action" in msg:
                if msg["action"] == "ping":
                    ...
                    if msg["success"] is False:
                        log(
                            "warning",
                            "Flovis data send failed ({}): {}".format(
                                msg["event_id"], msg["code"]
                            ),
                        )
            else:
                ws.send(
                    json.dumps(
                        {"action": "info", "message": "LA LA LA I'M NOT LISTENING"}
                    )
                )
        ...
        try:
            self.ws = websocket.WebSocketApp(
                self.host, on_message=on_message, on_close=on_close
            )
            ...
            def run():
                try:
                    ...
                    log(
                        "error",
                        "Flovis websocket closed unexpectedly, assuming problems and nullifying ws",
                    )
