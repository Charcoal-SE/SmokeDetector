...
def handle_commands(content_lower, message_parts, ev_room, ev_user_id, ev_user_name, wrap2):
    ...
    if content_lower.startswith("!!/so2015"):
        ...
        if election_start > now:
            ...
            return "Yay for the [2015 Stack Overflow Moderator Election](http://stackoverflow.com/election/6)! Election voting begins in %s %s, %s %s, %s %s and %s %s." % (diff.days, daystr, hours, hourstr, minutes, minutestr, seconds, secondstr)
            ...
            minutestr = "minutes" if minutes != 1 else "minute"
            secondstr = "seconds" if seconds != 1 else "second"
            return "Yay for the [2015 Stack Overflow Moderator Election](http://stackoverflow.com/election/6)! Election voting ends in %s %s, %s %s, %s %s and %s %s." % (diff.days, daystr, hours, hourstr, minutes, minutestr, seconds, secondstr)
        elif now < election_end:
            return "Congrats to Martijn Pieters, someone else, and someone else!"
