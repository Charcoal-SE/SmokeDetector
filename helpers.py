...
class ErrorLogs:
    ...
    def fetch_last(cls, n):
        db = cls.get_db()
        cursor = db.execute(
            "SELECT * FROM error_logs ORDER BY time DESC LIMIT ?", (int(n),)
        )
...
def redact_text(text, redact_str, replace_with):
    if redact_str:
        return (
            text.replace(redact_str, replace_with)
...
def log(log_level, *args, f=False):
    ...
    log_str = "{} {}".format(
        colored(
            "[{}]".format(datetime.utcnow().isoformat()[11:-3]), color, attrs=["bold"]
        ),
...
def log_exception(exctype, value, tb, f=False, *, level="error"):
    ...
    logged_msg = "{exception}\n{now} UTC\n{row}\n\n".format(
        exception=exception_only, now=now, row=tr
    )
...
def unshorten_link(url, request_type="GET", depth=10):
    ...
    response_code = 301
    headers = {
        "User-Agent": "SmokeDetector/git (+https://github.com/Charcoal-SE/SmokeDetector)"
    }
    for tries in range(depth):
        if response_code not in {301, 302, 303, 307, 308}:
            break
        res = requests.request(
            request_type, url, headers=headers, stream=True, allow_redirects=False
        )
