...
class GitHubManager:
    ...
    if still_using_usernames:
        ...
        auth_args = {
            "auth": HTTPBasicAuth(
                GlobalVars.github_username, GlobalVars.github_password
            )
        }
    else:
        still_using_usernames_nudge = ""
        auth_args = {
            "headers": {
                "Authorization": "token {}".format(GlobalVars.github_access_token)
            }
        }
    ...
    def comment_on_thread(cls, thread_id, body):
        url = "https://api.github.com/repos/{}/issues/{}/comments".format(
            GlobalVars.bot_repo_slug, thread_id
        )
...
class GitManager:
    ...
    def get_origin_or_auth():
        ...
        if (
            git_url[0:19] == "https://github.com/"
            and GlobalVars.github_username
            and GlobalVars.github_access_token
        ):
            preformat_url = "https://{}:{}@github.com/" + git_url[19:]
            return preformat_url.format(
                quote(GlobalVars.github_username), quote(GlobalVars.github_access_token)
            )
        else:
            return "origin"
    ...
    def add_to_blacklist(
        ...
        try:
            ...
            if code_permissions:
                ...
                if (
                    GlobalVars.github_username is None
                    or GlobalVars.github_password is None
    ...
    def remove_from_blacklist(
        ...
        if not code_privileged:
            if metasmoke_down:
                ...
                return (
                    False,
                    "Ask a blacklist manager to run that for you. Use `!!/whois blacklister` to find "
                    "out who's here.",
                )
    ...
    def merge_pull_request(cls, pr_id, comment=""):
        ...
        if pr_info["user"]["login"] != "SmokeDetector":
            raise ValueError(
                "PR #{} is not created by me, so I can't approve it.".format(pr_id)
            )
        if "<!-- METASMOKE-BLACKLIST" not in pr_info["body"]:
            raise ValueError("PR description is malformed. Blame a developer.")
        if pr_info["state"] != "open":
            raise ValueError(
                "PR #{} is not currently open, so I won't merge it.".format(pr_id)
            )
        ...
        try:
            ...
            git(
                ...
                "--no-ff",
                "-m",
                "Merge pull request #{} from {}/{}".format(
            ...
            try:
                ...
                pass
            return (
                "Merged pull request [#{0}](https://github.com/{1}/pull/{0}).".format(
    ...
    def reject_pull_request(cls, pr_id, comment=""):
        ...
        if pr_info["user"]["login"] != "SmokeDetector":
            raise ValueError(
                "PR #{} is not created by me, so I can't reject it.".format(pr_id)
            )
        if "<!-- METASMOKE-BLACKLIST" not in pr_info["body"]:
            raise ValueError("PR description is malformed. Blame a developer.")
        if pr_info["state"] != "open":
            raise ValueError(
                "PR #{} is not currently open, so I won't reject it.".format(pr_id)
            )
        ref = pr_info["head"]["ref"]
        ...
        raise RuntimeError(
            "Closing pull request #{} failed. Manual operations required.".format(pr_id)
        )
    ...
    def prepare_git_for_operation(blacklist_file_name):
        try:
            ...
            return (
                False,
                "`git pull` has failed. This shouldn't happen. Details have been logged.",
            )
        ...
        if local_ref != remote_ref:
            local_log = git.log(
                r"--pretty=`[%h]` *%cn*: %s", "-1", str(local_ref)
            ).strip()
            remote_log = git.log(
                r"--pretty=`[%h]` *%cn*: %s", "-1", str(remote_ref)
            ).strip()
            return (
                False,
                "HEAD isn't at tip of origin's master branch (local {}, remote {})".format(
    ...
    def sync_remote():
        try:
            ...
            return (
                True,
                "Synced to origin/master and origin/deploy. You'll probably want to !!/reboot now.",
            )
        except Exception as e:
            return False, str(e)
