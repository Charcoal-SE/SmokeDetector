    ...
    def remove(self, item: Union[str, dict]):
        ...
        with open(self._filename, "r+", encoding="utf-8") as f:
            ...
            items = [
                ...
                for x in items
                if ("\t" not in x)
                or (len(x.split("\t")) == 3 and x.split("\t")[2].strip() != item)
            ]
            f.seek(0)
            f.truncate()
            f.writelines(items)
...
class YAMLParserCIDR(BlacklistParser):
    ...
    def _write(self, callback):
        d = {
            ...
            "items": sorted(
                self._parse(keep_disabled=True), key=lambda x: x[self.SCHEMA_PRIKEY]
            ),
        }
        callback(d)
    ...
    def _validate(self, item):
        ...
        if "ip" in item:
            if not ip_regex.match(item["ip"]):
                raise ValueError(
                    'Field "ip" is not a valid IP address: {0}'.format(item["ip"])
                )
            """
            if 'cidr' in item:
                raise ValueError(
        ...
        def add_callback(d):
            for compare in d["items"]:
                if compare[prikey] == item[prikey]:
                    raise ValueError(
                        "{0} already in list {1}".format(item[prikey], d["items"])
                    )
            d["items"].append(item)
    ...
    def remove(self, item):
        ...
        def remove_callback(d):
            for i, compare in enumerate(d["items"]):
                ...
                raise ValueError(
                    "No {0} found in list {1}".format(item[prikey], d["items"])
                )
            del d["items"][i]
        ...
        def item_check(ns):
            ...
            try:
                ...
                if not item.get("pass", None):
                    ...
                    log(
                        "debug",
                        "{0} has no A record; SOA is {1}".format(
                ...
                if not item.get("pass", None):
                    log(
                        "warn",
                        "{0} has no available servers to service DNS "
                        "request.".format(ns),
                    )
