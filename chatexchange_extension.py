...
class Room(rooms.Room):
    def send_message(self, text, length_check=True):
        if "no-chat" in sys.argv:
            print("Blocked message to {0} due to no-chat setting: {1}".format(self.name, text))
            return

        if "charcoal-hq-only" not in sys.argv or int(self.id) == 11540:
            return rooms.Room.send_message(self, text, length_check)
        else:
            print("Blocked message to {0} due to charcoal-hq-only setting: {1}".format(self.name, text))

    def watch_socket(self, event_callback):
        if "no-chat" in sys.argv:
            print("Blocked socket connection to {0} due to no-chat setting".format(self.name))
            return
