...
class MetasmokeCache:
    ...
    def get(key):
        ...
        if key in MetasmokeCache._cache:
            if (
                key in MetasmokeCache._expiries and MetasmokeCache._expiries[key] >= int(time.time())
    ...
    def fetch(key, generator=None, expiry=None):
        ...
        if value is not None:
            ...
            if key in MetasmokeCache._prior_attempt_timestamp and MetasmokeCache._prior_attempt_timestamp[key] >= int(
                time.time() - MetasmokeCache.MINIMUM_SECONDS_BETWEEN_ATTEMPTS
            ):
...
def dump_cache_data():
    ...
    datahandling.dump_pickle(
        'metasmokeCacheData.p', {'cache': MetasmokeCache._cache, 'expiries': MetasmokeCache._expiries}
    )
