...
class Metasmoke:
    ...
    class AutoSwitch:
        ...
        def ping_failed():
            ...
            if (
                current_counter > Metasmoke.AutoSwitch.MAX_FAILURES
                and GlobalVars.MSStatus.is_up()
                and current_auto
            ):
                log(
                    "warning",
                    "Last {} connection(s) to metasmoke failed".format(current_counter)
                    + " Setting metasmoke status to down.",
                )
                chatcommunicate.tell_rooms_with(
                    "debug",
                    "**Warning**: {}: ".format(GlobalVars.location)
        ...
        def ping_succeeded():
            ...
            if (
                current_counter > Metasmoke.AutoSwitch.MAX_SUCCESSES
                and GlobalVars.MSStatus.is_down()
                and current_auto
            ):
                ...
                chatcommunicate.tell_rooms_with(
                    "debug",
                    "**Notice**: {}: ".format(GlobalVars.location)
                    + "Last {} connection(s) to metasmoke".format(current_counter)
        ...
        def enable_autoswitch(to_enable):
            ...
            with Metasmoke.AutoSwitch.rw_lock:
                if Metasmoke.AutoSwitch.autoswitch_is_on is not to_enable:
                    ...
                    switch_auto_msg = (
                        "Metasmoke status autoswitch is now {}abled.".format(
            ...
            try:
                ...
                if not has_succeeded:
                    ...
                    if (
                        ...
                        == MAX_MS_WEBSOCKET_RETRIES_TO_LONG_INTERVAL
                    ):
                        chatcommunicate.tell_rooms_with(
                            "debug",
                            "Cannot initiate MS websocket."
    ...
    def handle_websocket_data(data):
        ...
        if "message" in message:
            ...
            if from_ms.startswith(
                "[ [charcoal-se.github.io](https://github.com/Charcoal-SE/charcoal-se.github.io) ]"
                " continuous-integration/travis-ci/push"
            ):
                from_ms = from_ms.replace(
                    ": ",
                    ", or the [SD wiki](//git.io/vyDZv)"
                    " ([history](//github.com/Charcoal-SE/SmokeDetector/wiki/_history)): ",
                    1,
                )
            ...
            datahandling.add_blacklisted_user(
                ids, "metasmoke", message["blacklist"]["post"]
            )
            ...
            if sha != os.popen('git log -1 --pretty="%H"').read():
                if "autopull" in message["deploy_updated"]["head_commit"]["message"]:
                    if only_blacklists_changed(GitManager.get_remote_diff()):
                        commit_md = "[`{0}`](https://github.com/{1}/commit/{0})".format(
                            sha[:7], GlobalVars.bot_repo_slug
                        )
            ...
            sha = c["commit_sha"][:7]
            recent_commits = (
                sp.check_output(["git", "log", "-50", "--pretty=%H"])
                .decode("utf-8")
                .strip()
                .split("\n")
            )
            ...
            if c["status"] == "success":
                if (
                    "autopull" in c["commit_message"]
                    or c["commit_message"].startswith("!")
                    or c["commit_message"].startswith("Auto ")
                ):
                    s = (
                        "[CI]({ci_link}) on [`{commit_sha}`](https://github.com/{repo}/"
                    ...
                    s = (
                        "[CI]({ci_link}) on [`{commit_sha}`](https://github.com/{repo}/commit/{commit_sha}) "
                        "succeeded.".format(
                            ci_link=c["ci_url"],
                            repo=GlobalVars.bot_repo_slug,
                            commit_sha=sha,
    ...
    def send_stats_on_post(
        ...
        if GlobalVars.metasmoke_host is None:
            log(
                "info",
                "Attempted to send stats but metasmoke_host is undefined. Ignoring.",
            )
            return
        ...
        try:
            ...
            post = {
                ...
                "user_link": user_link,
                "why": why,
                "user_reputation": owner_rep,
                "score": post_score,
                "upvote_count": up_vote_count,
                "downvote_count": down_vote_count,
            }
    ...
    def send_feedback_for_post(post_link, feedback_type, user_name, user_id, chat_host):
        if GlobalVars.metasmoke_host is None:
            log(
                "info",
                "Received chat feedback but metasmoke_host is undefined. Ignoring.",
            )
            return
        elif GlobalVars.MSStatus.is_down():
    ...
    def send_deletion_stats_for_post(post_link, is_deleted):
        if GlobalVars.metasmoke_host is None:
            log(
                "info",
                "Attempted to send deletion data but metasmoke_host is undefined. Ignoring.",
            )
            return
        elif GlobalVars.MSStatus.is_down():
        ...
        try:
            payload = {
                "deletion_log": {"is_deleted": is_deleted, "post_link": post_link},
                "key": metasmoke_key,
            }

            headers = {"Content-type": "application/json"}
            Metasmoke.post(
                "/deletion_logs.json", data=json.dumps(payload), headers=headers
            )
        except Exception as e:
            log("error", e)
    ...
    def send_status_ping():
        if GlobalVars.metasmoke_host is None:
            ...
            payload = {"location": GlobalVars.location, "timestamp": time.time()}
    ...
    def update_code_privileged_users_list():
        if GlobalVars.MSStatus.is_down():
            log(
                "warning",
                "Metasmoke is down, can't update blacklist manager privilege list",
            )
            return
        ...
        headers = {"Content-type": "application/json"}
        try:
            response = Metasmoke.get(
                "/api/users/code_privileged", data=json.dumps(payload), headers=headers
            ).json()["items"]
        except Exception as e:
            log("error", e)
            return
    ...
    def determine_if_autoflagged(post_url):
        ...
        if len(response["items"]) > 0 and response["items"][-1]["autoflagged"]:
            ...
            flags = Metasmoke.get(
                "/api/v2.0/posts/" + id + "/flags", params=payload
            ).json()

            if len(flags["items"]) > 0:
                return True, [
                    user["username"]
                    for user in flags["items"][0]["autoflagged"]["users"]
                ]
    ...
    def stop_autoflagging():
        ...
        Metasmoke.post(
            "/flagging/smokey_disable", data=json.dumps(payload), headers=headers
        )
    ...
    def send_statistics():
        ...
        if GlobalVars.metasmoke_host is not None:
            ...
            Metasmoke.post(
                "/statistics.json", data=json.dumps(payload), headers=headers
            )
    ...
    def post_auto_comment(msg, user, url=None, ids=None):
        ...
        if url is not None:
            params = {
                "key": GlobalVars.metasmoke_key,
                "urls": url,
                "filter": "GFGJGHFJNFGNHKNIKHGGOMILHKLJIFFN",
            }
            ...
            try:
                response = Metasmoke.get(
                    "/api/v2.0/posts/uid/{}/{}".format(site, post_id), params=params
                ).json()
        ...
        if response and "items" in response and len(response["items"]) > 0:
            ...
            params = {
                "key": GlobalVars.metasmoke_key,
                "text": msg[:1].upper()
                + msg[1:],  # Capitalise the first letter of the comment
    ...
    def get_post_bodies_from_ms(post_url):
        if (
            not GlobalVars.metasmoke_key
            or not GlobalVars.metasmoke_host
            or GlobalVars.MSStatus.is_down()
        ):
            return None
        ...
        try:
            ...
            chatcommunicate.tell_rooms_with(
                "debug",
                "{}: In getting MS post information, recovered from `{}`".format(
                    GlobalVars.location, exception_only
                ),
            )
            return None
    ...
    def request_sender(method):
        def func(url, *args, ignore_down=False, **kwargs):
            if not GlobalVars.metasmoke_host or (
                GlobalVars.MSStatus.is_down() and not ignore_down
            ):
                return None
    ...
    def send_status_ping_and_verify_scanning_if_active():
        def reboot_or_standby(action):
            error_message = "There's been no scan activity for {} status pings. Going to {}.".format(
                Metasmoke.status_pings_since_scan_activity, action
            )
        ...
        if not in_standby_mode:
            ...
            if GlobalVars.PostScanStat.get_stat() == Metasmoke.scan_stat_snapshot:
                ...
                if (
                    Metasmoke.status_pings_since_scan_activity
                    >= NO_ACTIVITY_PINGS_TO_REBOOT
                ):
                    # Assume something is very wrong. Report to debug rooms and go into standby mode.
                    reboot_or_standby("reboot")
                elif (
                    Metasmoke.status_pings_since_scan_activity
                    >= NO_ACTIVITY_PINGS_TO_STANDBY
                ):
                    # Assume something is very wrong. Report to debug rooms and go into standby mode.
                    reboot_or_standby("standby")
                elif (
                    Metasmoke.status_pings_since_scan_activity
                    >= NO_ACTIVITY_PINGS_TO_REPORT
                ):
                    # Something might be wrong. Let people in debug rooms know.
                    status_message = "There's been no scan activity for {} status pings. There may be a problem.".format(
                        Metasmoke.status_pings_since_scan_activity
                    )
