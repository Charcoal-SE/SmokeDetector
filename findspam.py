...
def link_at_end(s, site):   # link at end of question, on selected sites
    ...
    if len(match) > 0 and not regex.compile(r"upload|\b(imgur|yfrog|gfycat|tinypic|ctrlv|prntscr|gyazo|youtu\.?be|stackexchange|superuser|past[ie].*|dropbox|microsoft|newegg|cnet|(?<!plus\.)google|localhost|ubuntu)\b", regex.UNICODE).search(match[0]):
...
class FindSpam:
    bad_keywords = ["baba ?ji", "fifa.{0,20}coins?", "fifabay", "Long Path Tool",
                    ...
                    "in (bangalore|chennai|delhi|hyderabad|kolkata|mumbai|madurai)</a>",
    ...
    bad_keywords_nwb = [u"ಌ", "vashi?k[ae]r[ae]n", "babyli(ss|cious)", "garcinia", "cambogia", "acai ?berr",  # "nwb" == "no word boundary"
                        ...
                        "(compan(y|ies)|training|courses?).{0,4}(bangalore|chennai|delhi|hyderabad|kolkata|mumbai|madurai)",
    ...
    blacklisted_websites = ["online ?kelas", "careyourhealths", "wowtoes", "(naga|dewa)poker",
                            ...
                            "mkmk9", "malwaretips", "hackerscontent\\.com", "healthyfinder\\.com",
                            ...
                            "customizedwallpaper\\.com", "cracksofts\\.com", "oathtohealth\\.com",
                            ...
                            "dcweddingandevents\\.com", "slimdreneavis"]
    # Patterns: the top three lines are the most straightforward, matching any site with this string in domain name
    pattern_websites = [r"(vipmodel|porn|wholesale|inboxmachine|(get|buy)cheap|escort|diploma|governmentjobs|extramoney|earnathome|spell(caster|specialist)|profits|seo(tool|service|trick)|onsale|fat(burn|loss)|(\.|//|best)cheap|online(training|solution))[\w-]*?\.(co|net|org|in\W|info|wordpress|blogspot)",
                        r"(truo?ng|viet|phone-?number|fullmovie|tvstream|trainingin|digitalmarketing|infocampus|cracked\w{3}|cracksoftware|bestmover|relocation|\w{4}mortgage|loans|revenue|testo[-bsx]|cleanse|cleansing|detox|supplement|lubricant|serum|wrinkle|topcare|freetrial)[\w-]*?\.(co|net|org|in\W|info|wordpress|blogspot)",
                        r"(nitricoxide|masculin|menhealth|babaji|spellcaster|potentbody|moist|lefair|lubricant|derma(?![nt])|xtrm|factorx|(?<!app)nitro(?!us)|crazy(bulk|mass)|nauseam|endorev|ketone|//xtra)[\w-]*?\.(co|net|org|in\W|info|wordpress|blogspot)",
    ...
    rules = [
        ...
        {'regex': ur"(?is)(>>>|===>|==>>|(Read More|Click Here) \W{2,20}).{0,20}http(?!://i.stack.imgur.com).{0,200}$", 'all': True,
        ...
        {'regex': ur"(?is)\b(nigg(a|er)|asshole|fag(got)?|daf[au][qk]|(mother)?fuc?k+(ing?|e?r)?|shit(t?er|head)|dickhead|pedo|whore|cunt|suck\b.{0,10}\bdick|dee[sz]e? nut[sz])s?\b|^.{0,250}\bshit\b.{0,250}$", 'all': True,
        ...
        {'regex': u"(?i)(tejveer ?iq|ser?vice pemanas?|Barbara Glowa|dlqudals|^[a-z ]+juriya[a-z]?$)", 'all': True, 'sites': [], 'reason': "blacklisted username", 'title': False, 'body': False, 'username': True, 'stripcodeblocks': False, 'body_summary': False, 'max_rep': 1, 'max_score': 0}
    ]
