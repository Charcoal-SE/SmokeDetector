...
def has_health(s, site):   # flexible detection of health spam in titles
    ...
    goal = regex.compile(r"(?i)\b(supple|build|los[es]|power|young(er)?|youth(ful)?|burn|erection|IQ|energy|tone(d)|shred(ed)?|rip(ped)?|bulk up|get rid|mood)s?\b|\b(diminish|look|reduc|beaut|renew|lift|eliminat|enhance|health|improve|enlarge|remov|vital|slim|lean|boost|str[oe]ng)s?").search(s)
...
class FindSpam:
    ...
    blacklisted_websites = ["online ?kelas", "careyourhealths", "wowtoes",
                            ...
                            "latestone\\.com", "iwebsoul\\.com", "uttarakhandshadi", "kaintek\\.com"]
    ...
    rules = [
        ...
        {'regex': ur"(?i)^(?:(?=.*?\b(?:online|hd)\b)(?=.*?(?:free|full|unlimited)).*?movies?\b)|(?=.*?\b(?:acai|kisn)\b)(?=.*?care).*products?\b|(?=.*?packer).*mover|(online|certification).*?training| vs .* (live|vivo)|\bxtra\b|\bwe offer\b|payday loan|смотреть онлайн", 'all': True,
