...
SE_SITES_DOMAINS = [
    ...
    "sstatic.net",
    "imgur.com",
]  # Frequently catching FP
WHITELISTED_WEBSITES_REGEX = regex.compile(
    r"(?i)upload|\b(?:{})\b".format(
        "|".join(
            [
                "yfrog",
                "gfycat",
...
URL_SHORTENER_REGEX_FRAGMENT = r"(?:{})".format(
    "|".join(
        regex.escape(site)
        for site in (
            "0i.is",
...
class PostFilter:
    ...
    def match(self, post):
        ...
        """
        if (post.is_answer and not self.answer) or (
            not post.is_answer and not self.question
        ):
            # Wrong post type
            return False
...
class Rule:
    ...
    def sanity_check(self):
        if not self.func and not self.regex:
            raise TypeError(
                "A rule must have either 'func' or 'regex' valid! : {}".format(
                    self.reason
                )
            )
    ...
    def match(self, post):
        ...
        if self.func:  # Functional check takes precedence over regex check
            if self.whole_post:
                matched_title, matched_username, matched_body, why_text = self.func(
                    post
                )
                result_title = (
                    matched_title,
                ...
                if self.username:
                    matched_username, why_text = self.func(
                        post.user_name, post.post_site
                    )
                    result_username = (
                        matched_username,
                        reason_username,
            ...
            try:
                ...
                compiled_regex = regex.compile(
                    self.regex, regex.UNICODE, city=city_list, ignore_unused=True
                )
            ...
            if (self.body and not post.body_is_summary) or (
                self.body_summary and post.body_is_summary
            ):
            ...
            raise TypeError(
                "To match, a rule must have either 'func' or 'regex' valid! : {}".format(
                    self.reason
                )
            )
...
class FindSpam:
    ...
    def reload_blacklists(cls):
        ...
        # (?w:\b) is also useful
        cls.rule_bad_keywords.regex = (
            r"(?is)(?:^|\b|(?w:\b))(?:{})(?:\b|(?w:\b)|$)|{}".format(
            ...
            [
                "Position{} {}: {}".format(
                    ...
                    ", ".join(
                        ...
                        if len(span) < 14
                        else ["{}-{}".format(a, b) for a, b in span[:12]]
                        + ["+{} more".format(len(span) - 12)]
                    ),
                    word,
                )
    ...
    *,
    ...
    if regex is not None:
        ...
        rule = Rule(
            ...
            skip_creation_sanity_check=skip_creation_sanity_check,
        )
        if not disabled:
        ...
        def decorator(func):
            if isinstance(func, Rule):
                func = (
                    func.func
                )  # Extract the real function from the created rule to allow multi-creation
                try:
                    func.__call__
                except AttributeError:
                    raise ValueError(
                        "This rule does not contain a function, can't recreate"
                    ) from None
            rule = Rule(
                func,
                reason=reason,
...
def is_whitelisted_website(url):
    ...
    return bool(
        WHITELISTED_WEBSITES_REGEX.search(url)
    ) or metasmoke_cache.is_website_whitelisted(url)
...
@create_rule(
    "misleading link", title=False, max_rep=11, max_score=1, stripcodeblocks=True
)
def misleading_link(s, site):
    ...
    try:
        ...
        parsed_text_fld_with_extra_subdomain = tld.get_tld(
            "foo." + parsed_text.fld, fix_protocol=True, as_object=True
        )
        if (
            parsed_text.fld == parsed_text.tld
            or parsed_text.tld != parsed_text_fld_with_extra_subdomain.tld
        ):
    ...
    if (
        ...
        and parsed_text.fld.split(".")[-1] in SAFE_EXTENSIONS
    ):
        return False, ""
    ...
    if (
        levenshtein(parsed_href.domain, parsed_text.domain) <= LEVEN_DOMAIN_DISTANCE
    ):  # Preempt
        return False, ""
    ...
    try:
        text_domain = unquote_plus(
            parsed_text.domain.encode("ascii").decode("idna")
        )  # people do post this, sad
    except ValueError:
...
def has_repeating_words(s, site):
    ...
    matcher = regex.compile(
        r"\b(?P<words>(?P<word>[a-z]+))(?:[][\s.,;!/\()+_-]+(?P<words>(?P=word))){4,}\b",
...
def has_few_characters(s, site):
    s = regex.sub(
        "</?(?:p|strong|em)>", "", s
    ).rstrip()  # remove HTML paragraph tags from posts
    uniques = len(set(s) - {"\n", "\t"})
    length = len(s)
    ...
    if any([t[0] <= length < t[1] and uniques <= t[2] for t in thresholds]):
        if uniques >= 5 and site == "math.stackexchange.com":
            ...
            return False, ""
        return True, "Contains {} unique character{}".format(
            uniques, "s" if uniques >= 2 else ""
        )
    return False, ""
...
def mostly_img(s, site):
    ...
    if s_len_img / len(s) > IMG_TXT_R_THRES:
        return True, "{:.4f} of the post is html image blocks".format(
            s_len_img / len(s)
        )
    return False, ""
...
# noinspection PyUnusedLocal,PyMissingTypeHints
@create_rule(
    "repeating characters in {}", stripcodeblocks=True, max_rep=10000, max_score=10000
)
def has_repeating_characters(s, site):
    ...
    if (
        len(match) / len(s) >= REPEATED_CHARACTER_RATIO
    ):  # Repeating characters make up >= 20 percent
        return True, "{}".format(
            ", ".join(
                [
                    "{}*{}".format(repr(match[0]), len("".join(match)))
                    for match in matches
                ]
            )
        )
...
def link_at_end(s, site):  # link at end of question, on selected sites
    ...
    match = regex.compile(
        r"(?i)https?://(?:[.A-Za-z0-9-]*/?[.A-Za-z0-9-]*/?|plus\.google\.com/"
        r"[\w/]*|www\.pinterest\.com/pin/[\d/]*)(?=</a>\s*$)"
    ).search(s)
...
def non_english_link(s, site):  # non-english link in short answer
    if len(s) < 600:
        links = regex.compile(
            r'nofollow(?: noreferrer)?">([^<]*)(?=</a>)', regex.UNICODE
        ).findall(s)
        for link_text in links:
            word_chars = regex.sub(r"(?u)\W", "", link_text)
            non_latin_chars = regex.sub(r"\w", "", word_chars)
            if len(word_chars) >= 1 and (
                (len(word_chars) <= 20 and len(non_latin_chars) >= 1)
                or (len(non_latin_chars) >= 0.05 * len(word_chars))
            ):
...
def mostly_non_latin(
    s, site
):  # majority of post is in non-Latin, non-Cyrillic characters
    word_chars = regex.sub(r"(?u)[\W0-9]|http\S*", "", s)
    non_latin_chars = regex.sub(
        r"(?u)\p{script=Latin}|\p{script=Cyrillic}", "", word_chars
    )
    if len(non_latin_chars) > 0.4 * len(word_chars):
        return True, "Text contains {} non-Latin characters out of {}".format(
            len(non_latin_chars), len(word_chars)
        )
    return False, ""
...
# noinspection PyUnusedLocal,PyMissingTypeHints
@create_rule(
    "phone number detected in {}",
    body=False,
    sites=["patents.stackexchange.com", "math.stackexchange.com", "mathoverflow.net"],
)
def has_phone_number(s, site):
    if regex.compile(
        r"(?i)\b(address(es)?|run[- ]?time|error|value|server|hostname|timestamp|warning|code|"
        r"(sp)?exception|version|chrome|1234567)\b",
        regex.UNICODE,
    ).search(s):
    ...
    s = regex.sub("[Ss]", "5", s)
    s = regex.sub("[Iil|]", "1", s)
    matched = regex.compile(
        r"(?<!\d)(?:\d{2}\s?\d{8,11}|\d\s{0,2}\d{3}\s{0,2}\d{3}\s{0,2}\d{4}|8\d{2}"
    ...
    test_formats = [
        ...
        "NG",
        None,
    ]  # ^ don't match parts of too long strings of digits
    for phone_number in matched:
        if regex.compile(r"^21474(672[56]|8364)|^192168|^3221225").search(phone_number):
            return (
                False,
                "",
            )  # error code or limit of int size, or 192.168 IP, or 0xC000000_ error code
        for testf in test_formats:
            try:
                z = phonenumbers.parse(phone_number, testf)
                if phonenumbers.is_possible_number(z) and phonenumbers.is_valid_number(
                    z
                ):
                    log(
                        "debug",
                        "Possible {}, Valid {}, Explain: {}".format(
...
def check_blacklisted_numbers(s, site):
    return check_numbers(
        s, GlobalVars.blacklisted_numbers, GlobalVars.blacklisted_numbers_normalized
    )


@create_rule(
    "potentially bad keyword in {}",
    body_summary=True,
    max_rep=32,
    max_score=1,
    stripcodeblocks=True,
...
def check_watched_numbers(s, site):
    return check_numbers(
        s, GlobalVars.watched_numbers, GlobalVars.watched_numbers_normalized
    )
...
def customer_support_phrase(s, site):  # flexible detection of customer service
    ...
    shortened = excluded[
        0:300
    ].lower()  # If applied to body, use just the beginning: otherwise many false positives.
    shortened = regex.sub(r"[^A-Za-z0-9\s]", "", shortened)  # deobfuscate
    phrase = regex.compile(
        r"(tech(nical)? support)|((support|service|contact|help(line)?) (telephone|phone|"
        r"number))"
    ).search(shortened)
    if phrase:
...
def scam_aimed_at_customers(
    s, site
):  # Scams aimed at customers of specific companies or industries
    s = s[
        0:300
    ].lower()  # if applied to body, the beginning should be enough: otherwise many false positives
    ...
    business = regex.compile(
        ...
        r"lexmark|mcafee|microsoft|norton|out[l1]ook|quickbooks|sage|windows?|yahoo)\b"
    ).search(s)
    digits = len(regex.compile(r"\d").findall(s))
    if business and digits >= 5:
        keywords = regex.compile(
            r"(?i)\b(customer|help|care|helpline|reservation|phone|recovery|service|support|"
            r"contact|tech|technical|telephone|number)\b"
        ).findall(s)
        if len(set(keywords)) >= 2:
            matches = ", ".join(["".join(match) for match in keywords])
            return True, u"Targeting *{}* customers. Keywords: *{}*".format(
                business.group(0), matches
            )
    return False, ""
...
def has_health(s, site):  # flexible detection of health spam in titles
    s = s[
        0:200
    ]  # if applied to body, the beginning should be enough: otherwise many false positives
    capitalized = (
        len(regex.compile(r"\b[A-Z][a-z]").findall(s)) >= 5
    )  # words beginning with uppercase letter
    organ = regex.compile(
        r"(?i)\b(colon|skin|muscle|bicep|fac(e|ial)|eye|brain|IQ|mind|head|hair|peni(s|le)|"
        r"breast|body|joint|belly|digest\w*)s?\b"
    ).search(s)
    condition = regex.compile(
        r"(?i)\b(weight|constipat(ed|ion)|dysfunction|swollen|sensitive|wrinkle|aging|"
        r"suffer|acne|pimple|dry|clog(ged)?|inflam(ed|mation)|fat|age|pound)s?\b"
    ).search(s)
    goal = regex.compile(
        r"(?i)\b(supple|build|los[es]|power|burn|erection|tone(d)|rip(ped)?|bulk|get rid|mood)s?\b|"
        r"\b(diminish|look|reduc|beaut|renew|young|youth|lift|eliminat|enhance|energ|shred|"
        r"health(?!kit)|improve|enlarge|remov|vital|slim|lean|boost|str[oe]ng)"
    ).search(s)
    remedy = regex.compile(
        r"(?i)\b(remed(y|ie)|serum|cleans?(e|er|ing)|care|(pro)?biotic|herbal|lotion|cream|"
        r"gel|cure|drug|formula|recipe|regimen|solution|therapy|hydration|soap|treatment|supplement|"
        r"diet|moist\w*|injection|potion|ingredient|aid|exercise|eat(ing)?)s?\b"
    ).search(s)
    boast = regex.compile(
        r"(?i)\b(most|best|simple|top|pro|real|mirac(le|ulous)|secrets?|organic|natural|perfect|"
        r"ideal|fantastic|incredible|ultimate|important|reliable|critical|amazing|fast|good)\b|"
        r"\b(super|hyper|advantag|benefi|effect|great|valu|eas[iy])"
    ).search(s)
    other = regex.compile(
        r"(?i)\b(product|thing|item|review|advi[cs]e|myth|make use|your?|really|work|tip|shop|"
        r"store|method|expert|instant|buy|fact|consum(e|ption)|baby|male|female|men|women|grow|"
        r"idea|suggest\w*|issue)s?\b"
    ).search(s)
    score = (
        4 * bool(organ)
        + 2 * bool(condition)
...
@create_rule(
    "pattern-matching product name in {}",
    body_summary=True,
    stripcodeblocks=True,
    answer=False,
    max_rep=12,
...
def pattern_product_name(s, site):
    ...
    total_words = sum(
        [
            n
            for n in [len(set([regex.sub(r"\d", "", w) for w in m])) for m in matches]
            if n >= 2
        ]
    )
    if total_words >= 3:
...
def keyword_email(s, site):  # a keyword and an email in the same post
    if (
        regex.compile("<pre>|<code>").search(s) and site == "stackoverflow.com"
    ):  # Avoid false positives on SO
        return False, ""
    keyword = regex.compile(
        r"(?i)(\b(?:training|we (will )?(offer|develop|provide)|sell|invest(or|ing|ment)|credit|"
        ...
        r"\$(?<!\$\$) ?[0-9,.]{4}(?!\(\w+(?:\.\w+)*:\d+\))(?![^\n]+[\n]+\s+at \w)|@qq\.com|"
        r"\b(?:герпес|муж|жена|доктор|болезн))"
    ).findall(s)
    keyword = [t[0] for t in keyword]
    email = regex.compile(
        r"(?<![=#/])\b[A-z0-9_.%+-]+\b(?:@|\s*\(?at\)?\s*)\b(?!(example|domain|site|foo|\dx)"
        r"(?:\.|\s*\(?dot\)?\s*)[A-z]{2,4})\b(?:[A-z0-9_.%+-]|\s*\(?dot\)?\s*)+\b"
        r"(?:\.|\s*\(?dot\)?\s*)[A-z]{2,4}\b"
    ).search(s)
    if keyword and email:
        return True, u"Keyword *{}* with email *{}*".format(
            ", ".join(keyword), email.group(0)
        )
    obfuscated_email = regex.compile(
        r"(?<![=#/])\b[A-z0-9_.%+-]+ *(?:@|\W*at\W*) *(g *mail|yahoo) *(?:\.|\W*dot\W*) *com\b"
    ).search(s)
...
def keyword_link(s, site):  # thanking keyword and a link in the same short answer
    ...
    if not link or is_whitelisted_website(link.group(0)):
        return False, ""
    praise = regex.compile(
        r"(?i)\b(?:nice|good|interesting|helpful|great|amazing) (?:article|blog|post|information)\b"
        r"|very useful"
    ).search(s)
    ...
    if link and keyword:
        return True, u"Keyword *{}* with link {}".format(
            keyword.group(0), link.group(0)
        )
    if link and thanks and praise:
        return True, u"Keywords *{}*, *{}* with link {}".format(
            thanks.group(0), praise.group(0), link.group(0)
        )
    return False, ""
...
def bad_link_text(s, site):  # suspicious text of a hyperlink
    ...
    keywords = regex.compile(
        ...
        r""
        r"(online|service|company|agency|repair|recovery|school|universit(?:y|ies)|college)|"
    ...
    support = regex.compile(
        r"(?i)(^| )(customer|care|helpline|reservation|phone|recovery|service|support|contact|"
        r"help|tech|technical|telephone|number)($| )"
    )
    for link_text in links:
        keywords_match = keywords.search(link_text)
        if keywords_match:
            return True, u"Bad keyword *{}* in link text".format(
                keywords_match.group(0).strip()
            )
        business_match = business.search(link_text)
        support_match = support.search(link_text)
        if business_match and support_match:
            return True, u"Bad keywords *{}*, *{}* in link text".format(
                business_match.group(0).strip(), support_match.group(0).strip()
            )
    return False, ""


@create_rule(
    "bad pattern in URL {}", title=False, body_summary=True, stripcodeblocks=True
)
def bad_pattern_in_url(s, site):
    ...
    matches = [
        x
        for x in matches
        if not regex.match(r"^https?://{0}".format(SE_SITES_RE), x[0])
    ]
    if matches:
...
def purge_cache(cachevar, limit):
    ...
    log(
        "debug",
        "purge_cache({0}): age of oldest entry is {1}".format(
            limit, now - cachevar[oldest[0]]["timestamp"]
        ),
    )
    log(
        "debug",
        "purge_cache({0}): oldest remaining entry is {1}".format(
...
def dns_query(label, qtype):
    ...
    try:
        ...
        if str(exc).startswith("None of DNS query names exist:"):
            ...
            log(
                "warning",
                "DNS error {0} (duration: {1})".format(exc, endtime - starttime),
            )
        return None
    endtime = datetime.utcnow()
    return answer
    ...
    if "pytest" in sys.modules:
        for nsentry in nslist:
            if isinstance(nsentry, list):
                for ns in nsentry:
                    assert ns.endswith("."), "Missing final dot on NS entry {0}".format(
                        ns
                    )
            else:
    ...
    for domain in set(domains):
        ...
        if ns is not None:
            ...
            for ns_candidate in nslist:
                if (
                    type(ns_candidate) is list and nameservers == set(ns_candidate)
                ) or any(
                    ns.endswith(".{0}".format(ns_candidate)) for ns in nameservers
                ):
...
def get_ns_ips(domain):
    ...
    """
    ns_ips = []
    dom = tld.get_tld(domain, fix_protocol=True, as_object=True, fail_silently=True)
    if not dom:
...
@create_rule(
    "potentially problematic NS configuration in {}",
    stripcodeblocks=True,
    body_summary=True,
)
def ns_is_host(s, site):
    ...
    for hostname in post_hosts(s, check_tld=True):
        ...
        if host_ips and set(get_ns_ips(get_domain(hostname, full=True))) == host_ips:
            return (
                True,
                "Suspicious nameservers: all IP addresses for {0} are in set {1}".format(
                    hostname, host_ips
                ),
            )
    return False, ""
...
@create_rule(
    "potentially bad NS for domain in {}",
    body_summary=True,
    stripcodeblocks=True,
    answer=False,
    sites=["stackoverflow.com"],
    ...
    for hostname in post_hosts(s, check_tld=True):
        ...
        if a is not None:
            ...
            for addr in set([str(x) for x in a]):
                log("debug", "IP: IP {0} for hostname {1}".format(addr, hostname))
                if addr in ip_list:
                    return True, "{0} suspicious IP address {1}".format(hostname, addr)
...
@create_rule(
    "potentially bad IP for hostname in {}", stripcodeblocks=True, body_summary=True
)
def watched_ip_for_url_hostname(s, site):
    return ip_for_url_host(s, site, GlobalVars.watched_cidrs)


@create_rule("bad IP for hostname in {}", stripcodeblocks=True, body_summary=True)
def bad_ip_for_url_hostname(s, site):
    return ip_for_url_host(s, site, GlobalVars.blacklisted_cidrs)
...
def asn_for_url_host(s, site, asn_list):
    for hostname in post_hosts(s, check_tld=True):
        if any(
            hostname == x
            or hostname.endswith("." + x)
            or metasmoke_cache.is_website_whitelisted(hostname)
            for x in ASN_WHITELISTED_WEBSITES
        ):
            log("debug", "Skipping ASN check for hostname {0}".format(hostname))
            continue
        a = dns_query(hostname, "a")
        if a is not None:
            for addr in set([str(x) for x in a]):
...
@create_rule(
    "potentially bad ASN for hostname in {}", body_summary=True, stripcodeblocks=True
)
...
@create_rule(
    "username similar to website in {}",
    title=False,
    body_summary=True,
    question=False,
    whole_post=True,
...
def username_similar_website(post):
    ...
    if sim_ratio >= SIMILAR_THRESHOLD:
        return (
            ...
            False,
            True,
            "Username `{}` similar to {}, ratio={}".format(
                username,
                ", ".join(
                    [
                        "*{}* at position {}-{}".format(
                            ...
                            body_lowercase.index(w.lower()) + len(w),
                        )
                        for w in sim_webs
                    ]
                ),
...
@create_rule(
    "single character over used in post",
    max_rep=22,
    body_summary=True,
    all=False,
...
def character_utilization_ratio(s, site):
    ...
    if highest_ratio > CHARACTER_USE_RATIO:
        return (
            True,
            "The `{}` character appears in a high percentage of the post".format(
                highest_char
            ),
        )
    else:
        ...
        if other_answer.post_id != post.post_id:
            ...
            if ratio >= SIMILAR_ANSWER_THRESHOLD:
                return (
                    ...
                    False,
                    True,
                    u"Answer similar to answer {}, ratio {}".format(
                        other_answer.post_id, ratio
                    ),
                )
...
def mostly_punctuations(s, site):
    ...
    if overall_frequency >= PUNCTUATION_RATIO:
        return True, u"Post contains {} marks of {!r} out of {} characters".format(
            num_punc, "".join(all_punc_set), len(s)
        )
    else:
        return False, ""
...
def toxic_check(post):
    ...
    if "error" in response:
        ...
        if not err_msg.startswith(
            ...
            log(
                "debug",
                "Perspective error: {} for string {} (original body {})".format(
                    err_msg, s, post.body
                ),
            )
    else:
...
if (
    GlobalVars.perspective_key
):  # don't bother if we don't have a key, since it's expensive
    toxic_check = create_rule(
        "toxic {} detected", func=toxic_check, whole_post=True, max_rep=101, max_score=2
    )
...
def body_starts_with_title(post):
    ...
    if post.is_answer or len(t) <= 10:
        log(
            "warning",
            "Length of post title is 10 characters or less. This is highly abnormal",
        )
        return False, False, False, ""
    ...
    if similar_ratio(s[: len(t)], t) >= BODY_TITLE_SIMILAR_RATIO:
        return (
            ...
            False,
            True,
            "Body starts with title and ends in URL: " + ending_url,
        )
...
@create_rule(
    ...
    max_rep=21,
    all=False,
    sites=["stackoverflow.com", "superuser.com"],
)
...
def turkey2(post):
    if regex.search("([01]{8}|zoe)", post.body):
        pingable = (
            chatcommunicate._clients["stackexchange.com"]
            .get_room(11540)
            .get_pingable_user_names()
        )
...
FindSpam.rule_bad_keywords = create_rule(
    "bad keyword in {}",
    regex="",
    username=True,
    body_summary=True,
...
FindSpam.rule_watched_keywords = create_rule(
    "potentially bad keyword in {}",
    regex="",
    username=True,
    body_summary=True,
...
FindSpam.rule_blacklisted_websites = create_rule(
    "blacklisted website in {}",
    regex="",
    body_summary=True,
    max_rep=52,
    max_score=5,
...
FindSpam.rule_blacklisted_usernames = create_rule(
...
create_rule(
    ...
    r"car\Win",
    all=False,
    sites=["superuser.com", "puzzling.stackexchange.com"],
    title=False,
    body=False,
    username=True,
)
...
create_rule(
    ...
    r"\bcorn\b",
    all=False,
    sites=["blender.stackexchange.com"],
    username=True,
)
...
create_rule(
    "Korean character in {}",
    r"(?i)\p{Script=Hangul}.*\p{Script=Hangul}.*\p{Script=Hangul}",
    sites=["korean.stackexchange.com"],
    body=False,
)
# Chinese characters in title: requires 3
create_rule(
    "Chinese character in {}",
    r"(?i)\p{Script=Han}.*\p{Script=Han}.*\p{Script=Han}",
    sites=[
        "chinese.stackexchange.com",
        "japanese.stackexchange.com",
        "ja.stackoverflow.com",
...
create_rule(
    "Hindi character in {}",
    r"(?i)\p{Script=Devanagari}",
    sites=["hinduism.stackexchange.com"],
    body=False,
)
# English text on non-English site: rus.SE only
create_rule(
    "English text in {} on a localized site",
    r"(?i)^[a-z0-9_\W]*[a-z]{3}[a-z0-9_\W]*$",
    all=False,
    sites=["rus.stackexchange.com"],
    stripcodeblocks=True,
)
# Roof repair
create_rule(
...
create_rule(
    "bad keyword in {}",
    r"(?i)serum(?<!truth serum)|\bsupplements(?<!to supplements)\b",
    all=False,
    sites=[
        "stackoverflow.com",
        "superuser.com",
...
create_rule(
    "bad keyword in {}",
    r"(?is)holocaust\W(witnesses|belie(?:f|vers?)|deni(?:er|al)|is\Wreal)",
    all=False,
...
create_rule(
    "bad keyword in {}",
    r"(?is)(?:^|\b|(?w:\b))(?:(?:poker|casino)\W*online"
    r"|online\W*(?:poker|casino))(?:\b|(?w:\b)|$)",
    all=True,
...
create_rule(
    ...
    r"JAMAL",
    all=False,
    sites=["codereview.stackexchange.com"],
    username=True,
    body_summary=True,
)
# Eggplant emoji
...
# Academia kangaroos
create_rule(
    "bad keyword in {}" r"(?i)kangaroos",
    all=False,
    sites=["academia.stackexchange.com"],
)
create_rule(
    'non-Google "google search" link in {}',
...
create_rule("title starts and ends with a forward slash", r"^\/.*\/$", body=False)
...
create_rule(
    ...
    r"،",
    all=False,
    sites=["politics.stackexchange.com", "politics.meta.stackexchange.com"],
    username=True,
    body_summary=False,
    body=False,
    title=False,
