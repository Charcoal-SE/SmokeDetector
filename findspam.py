...
def keyword_email(s, site):   # a keyword and an email in the same post
    keyword = regex.compile(ur"(?i)\b(we (offer|develop)|buy|sell|catalog|rent|crack|opportunity|candidate|loan|lover|illuminati|brotherhood|(join|reach) us|spell(caster)?|doctor|hack(er|ing)?|spying|passport|visa|seaman|scam|pics|vampire|bless(ed)?|atm|miracle|testimony|kidney|hospital|wetting)s?\b| Dr\.? ").search(s)
    if keyword:
...
class FindSpam:
    ...
    blacklisted_websites = ["online ?kelas", "careyourhealths", "wowtoes", "(naga|dewa)poker",
                            ...
                            "snipercrack\\.tk", "averagemaleheight\\.tk", "educba\\.com"]
    ...
    pattern_websites = [r"(wholesale|inboxmachine|(get|buy)cheap|escort|diploma|extramoney|earnathome|spell(caster|specialist)|profits|seo(tool|service|trick)|onsale|fat(burn|loss)|(\.|//|best)cheap|online(training|solution))[\w-]*?\.(co|net|org|in\W|info|wordpress|blogspot)",
                        r"(fullmovie|tvstream|trainingin|cracked\w{3}|cracksoftware|bestmover|relocation|\w{4}mortgage|loans|revenue|testo[-bsx]|cleanse|cleansing|detox|supplement|lubricant|serum|wrinkle|topcare|freetrial)[\w-]*?\.(co|net|org|in\W|info|wordpress|blogspot)",
    ...
    rules = [
        ...
        {'method': mostly_non_latin, 'all': True,
         ...
         'reason': 'mostly non-Latin {}', 'title': True, 'body': True, 'username': False, 'stripcodeblocks': True, 'body_summary': False, 'max_rep': 1, 'max_score': 0},
        #
        ...
        {'regex': ur'(?s)^.{0,350}\s*<a href="http://[\w.-]+/?"[^<]*</a>(?:</strong>)?\s*</p>\s*$', 'all': True,
