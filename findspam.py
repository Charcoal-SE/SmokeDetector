...
def link_at_end(s, site):   # link at end of question, on selected sites
    ...
    if len(match) > 0 and not regex.compile(r"upload|\b(imgur|yfrog|ctrlv|prntscr|gyazo|youtu\.?be|stackexchange|superuser|past[ie].*|dropbox|microsoft|newegg|cnet|(?<!plus\.)google|localhost|ubuntu)\b", regex.UNICODE).search(match[0]):
