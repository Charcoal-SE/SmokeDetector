...
class FindSpam:
    ...
    pattern_websites = [r"health\d{3,}", r"http\S*?\.repair\W", r"filefix(er)?\.com", "\.page\.tl\W",
                        ...
                        r"(bestmover|\w{4}mortgage|loans|revenue|escort|testo[-bsx]|cleanse|detox|supplement|lubricant|serum|wrinkle|topcare|freetrial)[\w-]*?\.(co|net|org|in\W|info)",
    ...
    rules = [
        ...
        {'regex': ur"(?i)\b(nigg(a|er)|asshole|fag(got)?|(mother)?fuc?k+(ing?|e?r)?|shit(t?er|hole|head)|dick(head)?|whore|cunt|dee[sz]e? nut[sz])s?\b", 'all': True,
