...
class FindSpam:
    ...
    rules = [
        ...
        {'regex': ur"(?i)(>>>>|===>|==>>>|Read More\s*>>)(?s).*http", 'all': True,
        ...
        {'regex': ur"(?i)diet ?plan|\b(pro)?derma(?!tolog)|(fat|weight) (loo?s[es]|reduction)|loo?s[es] weight", 'all': True,
        ...
        {'regex': ur"(?i)^(?:(?=.*?\b(?:online|hd)\b)(?=.*?(?:free|full|unlimited)).*?movies?\b|(?=.*?\b(?:acai|kisn)\b)(?=.*?care).*products?\b|(?=.*?packer).*mover)|online training", 'all': True,
        ...
        {'regex': ur"https?://(?!(www\.)?example\.(com|net|org))[a-zA-Z0-9_.-]+\.[a-zA-Z]{2,4}", 'all': True,
        ...
        {'regex': ur"^https?://(?!(www\.)?example\.(com|net|org))[a-zA-Z0-9_.-]+\.[a-zA-Z]{2,4}(/\S*)?$", 'all': False,
        ...
        {'regex': ur"(?<![=#/])\b[A-z0-9_.%+-]+@(?!example\.(com|net|org))[A-z0-9_.%+-]+\.[A-z]{2,4}\b", 'all': True,
        ...
        {'regex': ur"//[A-Za-z0-9-\.]*/?[A-Za-z0-9-]*/?</a>(?:</strong>)?\s*</p>\s*$", 'all': False, 'sites': ["superuser.com", "drupal.stackexchange.com", "meta.stackexchange.com"], 'reason': 'Link at end of {}', 'title': False, 'body': True, 'username': False, 'stripcodeblocks': False, 'body_summary': False, 'answers': False},
