...
<p><a href="http://example.com/" rel="nofollow">http://example.com/</a></p>''',
     '', '', True),
    ('',
     '''<p>Test <a href="http://example.com/" rel="nofollow">some text</a> moo moo moo.</p>
...
def test_check_if_spam_answer(title, body, username, site, match):
    # We can't check blacklists/whitelists in tests, so these are set to their default values
    user_url = ""
    post_id = 0
    # We *are* now testing answers separately
    is_answer = True
    is_spam, reason, _ = check_if_spam(title, body, username, user_url, site, post_id, is_answer, False, 1, 0)
    print title
    assert match == is_spam, 'expected spam on {0!r}'.format(body)
