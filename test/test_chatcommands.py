...
def test_coffee():
    ...
    assert regex.match(
        coffees + "@El'endiaStarman\\*", chatcommands.coffee(None, original_msg=msg)
    )
...
def test_tea():
    ...
    assert regex.match(
        teas + "@El'endiaStarman\\*", chatcommands.tea(None, original_msg=msg)
    )
...
def test_version():
    assert (
        chatcommands.version()
        == "{id} [{commit_name}]({repository}/commit/{commit_code})".format(
...
def test_bisect():
    ...
    msg = Fake(
        {
            "owner": {"name": "ArtOfCode", "id": 121520, "is_moderator": True},
            "room": {"_client": {"host": "stackexchange.com"}, "id": 11540},
            "content_source": None,
        }
    )
    ...
    ) == r"{!r} is not caught by a blacklist or watchlist item.".format(test_text)
    ...
    msg1 = Fake(
        {
            ...
            "room": {"get_current_user_ids": lambda: [161943]},
        }
    )

    assert (
        chatcommands.blame(original_msg=msg1)
...
def test_privileged():
    ...
    msg = Fake(
        {
            "owner": {"name": "ArtOfCode", "id": 121520, "is_moderator": False},
            "room": {"_client": {"host": "stackexchange.com"}, "id": 11540},
        }
    )

    assert (
        chatcommands.amiprivileged(original_msg=msg)
    ...
    assert (
        chatcommands.amiprivileged(original_msg=msg)
        == "\u2573 " + GlobalVars.not_privileged_warning
    )
...
def test_deprecated_blacklist():
    assert chatcommands.blacklist("").startswith(
...
def test_watch(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(
            "gitmanager.GitManager.prepare_git_for_operation",
        ...
        monkeypatch.setattr(
            "gitmanager.GitManager.add_to_blacklist",
...
def test_approve(monkeypatch):
    ...
    assert chatcommands.approve(8888, original_msg=msg).startswith(
        "You need blacklist manager privileges"
    )

    monkeypatch.setattr(
        GlobalVars, "code_privileged_users", [("stackexchange.com", 121520)]
    )
    with monkeypatch.context() as m:
        ...
        assert (
            chatcommands.approve(8888, original_msg=msg)
...
def test_reject(monkeypatch):
    ...
    assert chatcommands.reject(
        '8888 "test"', original_msg=msg, alias_used="reject"
    ).startswith("You need blacklist manager privileges")
    ...
    with monkeypatch.context() as m:
        ...
        assert (
            chatcommands.reject(
                '8888 "test"', original_msg=msg, alias_used="reject-force"
            )
            == "Cannot connect to GitHub API"
        )
        m.setattr("requests.get", original_get)
    assert chatcommands.reject(
        '2518 "test"', original_msg=msg, alias_used="close"
    ).startswith("Please provide")
...
def test_report(handle_spam):
    ...
    try:
        msg = Fake(
            {
                ...
                "room": {
                    "id": 11540,
                    "name": "Charcoal HQ",
                    "_client": {"host": "stackexchange.com"},
                },
                "_client": {"host": "stackexchange.com"},
                "id": 1337,
            }
        ...
        assert (
            chatcommands.report("test", original_msg=msg, alias_used="report")
        ...
        assert (
            chatcommands.report("https://stackoverflow.com/q/1", original_msg=msg)
        ...
        assert (
            chatcommands.report(test_post_url, original_msg=msg, alias_used="scan")
            is None
        )
        ...
        assert call["why"].startswith(
            "Post manually scanned by user *ArtOfCode* in room *Charcoal HQ*."
        )
        ...
        assert (
            chatcommands.report(
                test_post_url, original_msg=msg, alias_used="report-direct"
            )
            is None
        )
        _, call = handle_spam.call_args_list[-1]
        ...
        assert chatcommands.report(
            "https://stackoverflow.com/a/1732454", original_msg=msg
        ).startswith("Post 1: Already recently reported")
...
def test_allspam(handle_spam):
    try:
        ...
        assert (
            chatcommands.allspam("test", original_msg=msg)
            == "That doesn't look like a valid user URL."
        )

        # If this code lasts long enough to fail, I'll be happy
        assert (
            chatcommands.allspam(
                "https://stackexchange.com/users/10000000000", original_msg=msg
            )
            == "The specified user does not appear to exist."
        )

        assert chatcommands.allspam(
            "https://stackexchange.com/users/5869449", original_msg=msg
        ) == (
        ...
        assert chatcommands.allspam(
            "https://stackexchange.com/users/11683", original_msg=msg
        ) == (
        ...
        assert chatcommands.allspam(
            "https://stackoverflow.com/users/22656", original_msg=msg
        ) == (
            "The specified user's reputation is abnormally high. Please consider flagging for moderator attention, "
            "otherwise use !!/report on the posts individually."
        )
        ...
        assert (
            chatcommands.allspam(
                "https://stackexchange.com/users/12108751", original_msg=msg
            )
        ...
        assert (
            chatcommands.allspam(
                "https://stackoverflow.com/users/8846458", original_msg=msg
            )
        ...
        assert chatcommands.allspam(
            "https://stackoverflow.com/users/7052649", original_msg=msg
        ) == (
        ...
        assert (
            chatcommands.allspam(
                "https://stackexchange.com/users/12108974", original_msg=msg
            )
            is None
        )
        ...
        assert (
            call["why"]
            == "User manually reported by *ArtOfCode* in room *Charcoal HQ*.\n"
        )

        handle_spam.reset_mock()
        assert (
            chatcommands.allspam(
                "https://meta.stackexchange.com/users/373807", original_msg=msg
            )
            is None
        )
...
@pytest.mark.skipif(
    os.path.isfile("blacklistedUsers.p"), reason="shouldn't overwrite file"
)
def test_blacklisted_users():
    try:
        msg = Fake(
            {
                "owner": {"name": "ArtOfCode", "id": 121520, "is_moderator": False},
                "room": {"id": 11540, "_client": {"host": "stackexchange.com"}},
                "_client": {"host": "stackexchange.com"},
                "id": 1337,
            }
        )
        ...
        assert (
            chatcommands.isblu(
                "https://stackoverflow.com/users/4622463/angussidney", original_msg=msg
            )
        ...
        assert (
            chatcommands.isblu("4622463 stackoverflow", original_msg=msg)
            == "User is not blacklisted (`4622463` on `stackoverflow.com`)."
        )
        assert (
            chatcommands.addblu("4622463 stackoverflow", original_msg=msg)
            == "User blacklisted (`4622463` on `stackoverflow.com`)."
        )
        ...
        assert (
            chatcommands.rmblu("4622463 stackoverflow", original_msg=msg)
            == "User removed from blacklist (`4622463` on `stackoverflow.com`)."
        )
        assert (
            chatcommands.isblu("4622463 stackoverflow", original_msg=msg)
        ...
        assert (
            chatcommands.addblu(
                "https://meta.stackexchange.com/users", original_msg=msg
            )
            == "Invalid format. Valid format: `!!/addblu profileurl` *or* `!!/addblu userid sitename`."
        )
        assert (
            chatcommands.rmblu("https://meta.stackexchange.com/", original_msg=msg)
            == "Invalid format. Valid format: `!!/rmblu profileurl` *or* `!!/rmblu userid sitename`."
        )
        assert (
            chatcommands.isblu("msklkldsklaskd", original_msg=msg)
            == "Invalid format. Valid format: `!!/isblu profileurl` *or* `!!/isblu userid sitename`."
        )

        # Invalid sitename
        assert (
            chatcommands.addblu("1 completelyfakesite", original_msg=msg)
            == "Error: Could not find the given site."
        )
        assert (
            chatcommands.isblu("1 completelyfakesite", original_msg=msg)
            == "Error: Could not find the given site."
        )
        assert (
            chatcommands.rmblu("1 completelyfakesite", original_msg=msg)
            == "Error: Could not find the given site."
        )
    finally:
        # Cleanup
...
@pytest.mark.skipif(
    os.path.isfile("whitelistedUsers.p"), reason="shouldn't overwrite file"
)
def test_whitelisted_users():
    try:
        msg = Fake(
            {
                "owner": {"name": "El'endia Starman", "id": 1, "is_moderator": False},
                "room": {"id": 11540, "_client": {"host": "stackexchange.com"}},
                "_client": {"host": "stackexchange.com"},
            }
        )
        ...
        assert (
            chatcommands.iswlu("4622463 stackoverflow", original_msg=msg)
            == "User is not whitelisted (`4622463` on `stackoverflow.com`)."
        )
        assert (
            chatcommands.addwlu("4622463 stackoverflow", original_msg=msg)
            == "User whitelisted (`4622463` on `stackoverflow.com`)."
        )
        # TODO: Add test here as well
        assert (
        ...
        assert (
            chatcommands.rmwlu("4622463 stackoverflow", original_msg=msg)
            == "User removed from whitelist (`4622463` on `stackoverflow.com`)."
        )
        assert (
            chatcommands.iswlu("4622463 stackoverflow", original_msg=msg)
        ...
        assert (
            chatcommands.rmwlu("https://meta.stackexchange.com/", original_msg=msg)
            == "Invalid format. Valid format: `!!/rmwlu profileurl` *or* `!!/rmwlu userid sitename`."
        )
        assert (
            chatcommands.iswlu("msklkldsklaskd", original_msg=msg)
            == "Invalid format. Valid format: `!!/iswlu profileurl` *or* `!!/iswlu userid sitename`."
        )

        # Invalid sitename
        assert (
            chatcommands.addwlu("1 completelyfakesite", original_msg=msg)
            == "Error: Could not find the given site."
        )
        assert (
            chatcommands.iswlu("1 completelyfakesite", original_msg=msg)
            == "Error: Could not find the given site."
        )
    except:
        # Cleanup
...
def test_metasmoke():
    ...
    assert chatcommands.metasmoke(
        original_msg=msg, alias_used="ms-status"
    ) == msg_source.format("up", 0)
    chatcommunicate.tell_rooms_with = orig_tell_rooms_with


@pytest.mark.skipif(
    os.path.isfile("notifications.p"), reason="shouldn't overwrite file"
)
def test_notifications():
    try:
        ...
        msg2 = Fake(
            {
                "owner": {"name": "angussidney", "id": 145827, "is_moderator": False},
                "room": {"id": 11540, "_client": {"host": "stackexchange.com"}},
                "_client": {"host": "stackexchange.com"},
            }
        )
        ...
        # User 1
        assert (
            chatcommands.allnotificationsites("11540", original_msg=msg1)
            == "You won't get notified for any sites in that room."
        )
        assert (
            chatcommands.willbenotified("11540", "gaming", original_msg=msg1)
            == "No, you won't be notified for that site in that room."
        )
        assert (
            chatcommands.notify("11540", "gaming", None, original_msg=msg1)
            == "You'll now get pings from me if I report a post on `gaming`, in room `11540` on `chat.stackexchange.com`"
        )
        assert (
            chatcommands.notify(
                "11540", "codegolf.stackexchange.com", None, original_msg=msg1
            )
            == "You'll now get pings from me if I report a post on `codegolf.stackexchange.com`, in room `11540` on "
            "`chat.stackexchange.com`"
        )
        assert (
            chatcommands.willbenotified(
                "11540", "gaming.stackexchange.com", original_msg=msg1
            )
            == "Yes, you will be notified for that site in that room."
        )
        assert (
            chatcommands.willbenotified("11540", "codegolf", original_msg=msg1)
            == "Yes, you will be notified for that site in that room."
        )

        # User 2
        assert (
            chatcommands.allnotificationsites("11540", original_msg=msg2)
            == "You won't get notified for any sites in that room."
        )
        assert (
            chatcommands.willbenotified("11540", "raspberrypi", original_msg=msg2)
            == "No, you won't be notified for that site in that room."
        )
        assert (
            chatcommands.notify("11540", "raspberrypi", None, original_msg=msg2)
        ...
        assert (
            chatcommands.willbenotified(
                "11540", "raspberrypi.stackexchange.com", original_msg=msg2
            )
            == "Yes, you will be notified for that site in that room."
        )
        ...
        assert (
            chatcommands.unnotify("11540", "codegolf", original_msg=msg1)
            == "I will no longer ping you if I report a post on `codegolf`, in room `11540` on `chat.stackexchange.com`"
        )
        assert (
            chatcommands.unnotify("11540", "raspberrypi", original_msg=msg2)
        ...
        assert (
            chatcommands.allnotificationsites("asdf", original_msg=msg1)
            == "Invalid input type given for an argument"
        )
        assert (
            chatcommands.notify(
                "11540", "charcoalspam.stackexchange.com", None, original_msg=msg1
            )
            == "The given SE site does not exist."
        )

        assert (
            chatcommands.notify("11540", "codegolf", "True", original_msg=msg1)
            == "You'll now get pings from me if I report a post on `codegolf`, in room `11540` on `chat.stackexchange.com`"
        )
        assert (
            chatcommands.notify("11540", "codegolf", "False", original_msg=msg1)
            == "That notification configuration is already registered."
        )
    finally:
        # Cleanup
...
def test_inqueue():
    ...
    assert (
        chatcommands.inqueue("https://codegolf.stackexchange.com/a/1")
        == "Can't check for answers."
    )
    assert chatcommands.inqueue("https://stackoverflow.com/q/1") == "Not in queue."
    assert (
        chatcommands.inqueue("https://codegolf.stackexchange.com/q/1") == "#1 in queue."
    )
