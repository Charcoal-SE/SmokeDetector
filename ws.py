...
if os.path.isfile("plugin.py"):
    try:
        ...
        error_msg = "{}: {}\n{}".format(
            exc_type.__name__, exc_obj, traceback.format_tb(exc_tb)
        )
...
if THIS_PYTHON_VERSION < MIN_PYTHON_VERSION:
    msg = "SmokeDetector requires Python version {0}.{1}.{2} or newer to run.".format(
        *MIN_PYTHON_VERSION
    )
...
if THIS_PYTHON_VERSION < RECOMMENDED_PYTHON_VERSION:
    msg = (
        ...
        "SmokeDetector to use Python {0}.{1}.{2} or newer.".format(
...
def restart_automatically():
    ...
    chatcommunicate.tell_rooms_with(
        "debug", "{}: Executing automatic scheduled reboot.".format(GlobalVars.location)
    )
    time.sleep(6)
    exit_mode("reboot")
...
try:
    ...
    if "permission denied:" in strerror.lower():
        ...
        if (
            "/home/" in strerror
            and ".local/lib/python" in strerror
            and "/site-packages/tld/" in strerror
        ):
            err_msg = (
...
if "ChatExchangeU" in os.environ:
    ...
    log(
        "error",
        "No ChatExchange username provided. Set it in config or provide it via environment variable",
    )
    exit_mode("shutdown")
...
if "ChatExchangeP" in os.environ:
    ...
    log(
        "error",
        "No ChatExchange password provided. Set it in config or provide it via environment variable",
    )
    exit_mode("shutdown")
...
def check_socket_connections():
    for client in chatcommunicate._clients.values():
        if (
            ...
            and (datetime.utcnow() - client.last_activity).total_seconds() >= 60
        ):
...
def setup_websocket(attempt, max_attempts):
    try:
        ...
        log(
            "warning",
            "WS failed to create SE websocket connection. Attempt {} of {}.".format(
...
if "first_start" in sys.argv:
    chatcommunicate.tell_rooms_with(
        "debug", GlobalVars.s if GlobalVars.on_branch else GlobalVars.s_reverted
    )
    ...
    try:
        ...
        if a is not None and a != "":
            ...
            if action == "155-questions-active":
                ...
                t = Thread(
                    name="bodyfetcher post enqueing",
        ...
        if seconds < 180 and exc_type not in {
        ...
        if not GlobalVars.no_se_activity_scan:
            ws = init_se_websocket_or_reboot(
                MAX_SE_WEBSOCKET_RETRIES, tell_debug_room_on_error=True
            )

        chatcommunicate.tell_rooms_with(
            "debug",
            "{}: SE WebSocket: recovered from `{}`".format(
