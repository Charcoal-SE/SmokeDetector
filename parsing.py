...
def fetch_post_url_from_msg_content(content):
    search_regex = r"^\[ \[SmokeDetector\]\(\/\/git.io\/vgx7b\) \] [\w\s,:\(\)-]+: \[.+]\(((?:http:)?\/\/[\w.]+\/questions\/\d+(?:\/.*)?|(?:http:)?\/\/[\w.]+\/[qa]\/\d+/?)\) by \[?.*\]?\(?(?:.*)\)? on `[\w.]+`(?: \(@.+\))?(?: \[.+\]\(.+\))?$"
...
def fetch_user_from_allspam_report(content):
    search_regex = r"^\[ \[SmokeDetector\]\(\/\/git.io\/vgx7b\) \] All of this user's posts are spam: \[user \d+ on [\w\.]+\]\((//[\w\.]+/users/\d+\D*)\)(?: \[.+\]\(.+\))?$"
    m = regex.compile(search_regex).search(content)
    if m is None:
        return None
    try:
