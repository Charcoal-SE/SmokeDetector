...
def addblu(msg, user):
    ...
    if int(uid) > -1 and val != "":
        message_url = "https://chat.{}/transcript/{}?m={}".format(
            msg._client.host, msg.room.id, msg.id
        )
...
def minimally_validate_content_source(msg):
    ...
    if msg.content.split(" ")[0] != msg.content_source.split(" ")[0]:
        raise CmdException(
            "There was a problem with this command. Was the chat message edited or deleted?"
        )
...
def get_pattern_from_content_source(msg):
    ...
    try:
        ...
        # https://chat.stackexchange.com/transcript/11540?m=54465107#54465107
        raise CmdException(
            "An invalid pattern was provided, please check your command. Was the command edited?"
        )
...
def check_blacklist(string_to_test, is_username, is_watchlist, is_phone):
    ...
    if is_phone:
        filter_out.extend(
            ["mostly non-latin", "phone number detected", "messaging number detected"]
        )
...
def get_test_text_from_regex(pattern):
    ...
    pattern = pattern.replace(
        r"(?:[\W_]*+(?:at[\W_]*+)?gmail(?:[\W_]*+(?:dot[\W_]*+)?com)?)?", "@gmail.com"
    )
    pattern = regex.sub(
        r"(?<!\\)\(\?\#[^\)]*\)", "", pattern
    )  # Remove comments: comments can have no nested ()
    pattern = regex.sub(r"(?<!\\)\[\\W_\][*?+]*", "-", pattern)  # Replace typical sets
    pattern = regex.sub(
        r"(?<!\\)\[\\da-f\]\{4,\}+?", "0123", pattern
    )  # Replace typical sets
    pattern = regex.sub(
        r"(?<!\\)\[\\da-f\][*?+]*", "0", pattern
    )  # Replace typical sets
    pattern = regex.sub(
...
def do_blacklist(blacklist_type, msg, force=False):
    ...
    chat_user_profile_link = "https://chat.{host}/users/{id}".format(
        host=msg._client.host, id=msg.owner.id
    )
    append_force_to_do = (
        " Append `-force` to the command if you really want to add this pattern."
    )
    ...
    if "number" not in blacklist_type:
        ...
        test_for_unescaped_dot = regex.sub(
            r"(?<!\\)\(\?\#[^\)]*\)", "", pattern
        )  # remove comments
        # Remove character sets, where . doesn't need to be escaped.
        test_for_unescaped_dot = regex.sub(
            r"(?<!\\)\[(?:[^\]]|(?<=\\)\])*\]", "", test_for_unescaped_dot
        )
        if regex.search(r"(?<!\\)\.", test_for_unescaped_dot):
            other_issues.append(
                'The regex contains an unescaped "`.`", which should be "`\\.`" in most cases.'
            )
        ...
        try:
            ...
            raise CmdException(
                "An invalid pattern was provided, please check your command."
            )
    ...
    if code_permissions and only_blacklists_changed(GitManager.get_local_diff()):
        try:
            if not GlobalVars.on_branch:
                ...
                log(
                    "warning",
                    "Pulling local with HEAD detached, checkout deploy",
                    f=True,
                )
...
def watch(msg, pattern, alias_used="watch"):
    ...
    return do_blacklist(
        "watch_number" if "number" in alias_used else "watch_keyword",
        msg,
...
def approve(msg, pr_id):
    ...
    if not code_permissions:
        raise CmdException(
            "You need blacklist manager privileges to approve pull requests"
        )
...
def reject(msg, args, alias_used="reject"):
    ...
    if not code_permissions:
        raise CmdException(
            "You need blacklist manager privileges to reject pull requests"
        )
    if len(reason) < 20 and not force:
        raise CmdException(
            "Please provide an adequate reason for rejection (at least 20 characters long) so the user"
...
def blame(msg):
    unlucky_victim = msg._client.get_user(
        random.choice(msg.room.get_current_user_ids())
    )
...
def blame2(msg, x):
    ...
    try:
        user = sum(
            [(len(base) ** i) * base[char] for i, char in enumerate(reversed(x))]
        )
...
def coffee(msg, other_user):
    ...
    if other_user is None:
        return "*brews a cup of {} for @{}*".format(
            random.choice(COFFEES), msg.owner.name.replace(" ", "")
        )
    else:
...
def tea(msg, other_user):
    ...
    if other_user is None:
        return "*brews a cup of {} tea for @{}*".format(
            random.choice(TEAS), msg.owner.name.replace(" ", "")
        )
    else:
        other_user = regex.sub(r"^@*|\b\s.{1,}", "", other_user)
...
def block(msg, block_time, room_id):
    ...
    which_room = (
        "globally"
        if room_id is None
        else "in room {} on {}".format(room_id, msg._client.host)
    )
    block_message = "Reports blocked for {} second(s) {}.".format(
        time_to_block, which_room
    )
    tell_rooms(
        block_message, ((msg._client.host, msg.room.id), "debug", "metatavern"), ()
    )
...
def unblock(msg, room_id):
    ...
    tell_rooms(
        unblock_message, ((msg._client.host, msg.room.id), "debug", "metatavern"), ()
    )
...
@command(
    whole_msg=True,
    aliases=["ms-status", "ms-down", "ms-up", "ms-down-force", "ms-up-force"],
    give_name=True,
)
...
def welcome(msg, other_user):
    ...
    if other_user is None:
        raise CmdException(
            w_msg.format(room=msg.room.name, user="", me=GlobalVars.chatmessage_prefix)
        )
    else:
        other_user = regex.sub(r"^@*|\b\s.{1,}", "", other_user)
        raise CmdException(
            w_msg.format(
                room=msg.room.name,
                user=" @" + other_user,
                me=GlobalVars.chatmessage_prefix,
            )
        )
...
def pull(alias_used="pull"):
    ...
    request = requests.get(
        "https://api.github.com/repos/{}/git/refs/heads/deploy".format(
    ...
    if "success" in states:
        ...
        raise CmdException(
            "CI build is still pending, wait until the build has finished and then pull again."
        )
...
@command(
    ...
    privileged=True,
    give_name=True,
    aliases=["pull-sync", "pull-sync-force"],
)
def sync_remote(msg, alias_used="pull-sync"):
    ...
    if not is_code_privileged(msg._client.host, msg.owner.id):
        raise CmdException(
            "You don't have blacklist manager privileges to run this command."
        )
    if "force" not in alias_used:
        raise CmdException(
            "This command is deprecated, append `-force` if you really need to do that."
        )
...
def sync_remote_hard(msg, alias_used="pull-sync-hard"):
    ...
    if not is_code_privileged(msg._client.host, msg.owner.id):
        raise CmdException(
            "You don't have blacklist manager privileges, which are required to run this command."
        )
...
@command(
    privileged=True,
    give_name=True,
    aliases=["gitstatus", "git-status", "git-help", "git-merge-abort", "git-reset"],
)
...
def reboot(msg, alias_used="reboot"):
    ...
    if alias_used in {"reboot", "restart"}:
        tell_rooms(
            "{}: Goodbye, cruel world".format(GlobalVars.location),
...
@command(
    ...
    aliases=[
        "amicodeprivileged",
        "amiblacklisterprivileged",
        "amiblacklistmanagerprivileged",
    ],
)
...
def stappit(msg, location_search):
    ...
    """
    if (
        location_search is None
        or location_search.lower() in GlobalVars.location.lower()
    ):
        tell_rooms(
            "{}: Goodbye, cruel world".format(GlobalVars.location),
...
def status():
    ...
    return "Running since {time} UTC ({relative})".format(
        time=GlobalVars.startup_utc, relative=td_format(diff)
    )
...
def stopflagging(msg):
    ...
    log(
        "warning",
        "Disabling autoflagging ({} ran !!/stopflagging, message {})".format(
            msg.owner.name, msg.id
        ),
    )
    return "Stopping"
...
# noinspection PyIncorrectDocstring,PyProtectedMember
@command(
    str, whole_msg=True, privileged=True, aliases=["standby-except"], give_name=True
)
...
def test(content, alias_used="test"):
    ...
    if alias_used in ["test-q", "test-question"]:
        ...
        right_types = list(
            filter(
                lambda p: p[0] in json_obj and isinstance(json_obj[p[0]], p[1]),
                valid_keys,
            )
        )
        wrong_types = list(
            filter(
                lambda p: p[0] in json_obj and not isinstance(json_obj[p[0]], p[1]),
                valid_keys,
            )
        )
        ...
        if len(wrong_types) > 0:
            raise CmdException(
                "Invalid type: {}".format(
                    ", ".join(
                        [
                            "{} should be {}".format(x, y.__name__)
                            for (x, y) in wrong_types
                        ]
                    )
                )
            )
        ...
        fake_response = {
            ...
            "owner": {
                "display_name": json_obj["username"]
...
def bisect_regex(s, regexes, bookend=True, timeout=None):
    regex_to_format = (
        r"(?is)(?:^|\b|(?w:\b))(?:{})(?:$|\b|(?w:\b))" if bookend else r"(?i)(?:{})"
    )
    formatted_regex = regex_to_format.format("|".join([r for r, i in regexes]))
    start_time = time.time()
    try:
        compiled = regex.compile(
            formatted_regex, city=findspam.city_list, ignore_unused=True
        )
        ...
        log(
            "error",
            "bisect_regex: in {} seconds, got an error with the regex: {}".format(
                seconds, formatted_regex
            ),
        )
        raise
    ...
    if seconds > 0.1:
        ...
        log(
            "debug",
            "bisect_regex: took {} seconds for the regex: {}".format(
                seconds, formatted_regex
            ),
        )
    if not match:
    ...
    mid = 2 ** mid_len
    return bisect_regex(
        s, regexes[:mid], bookend=bookend, timeout=timeout
...
def bisect_regex_one_by_one(test_text, regexes, bookend=True, timeout=None):
    ...
    for expresion in regexes:
        compiled = regex.compile(
            regex_to_format.format(expresion[0]),
...
def bisect(msg, s):
    ...
    matching = bisect_regex_in_n_size_chunks(
        64, s, bookended_regexes, bookend=True, timeout=1
    )
    matching.extend(
        bisect_regex_in_n_size_chunks(
            64, s, non_bookended_regexes, bookend=False, timeout=1
        )
    )
...
def bisect_number(msg, s):
    ...
    if verbatim_match:
        ...
        if normalized_match:
            l, f = verbatim_match
            response = (
                "Matched verbatim on line {0} of {1}, and also normalized on".format(
                    l, f
                )
            )
        ...
        if len(normalized_match) == 1:
            ...
            return (
                "Not matched verbatim, but normalized on "
...
def threads():
    ...
    threads_list = [
        "{ident}: {name}".format(ident=t.ident, name=t.name)
...
def whoami(msg):
    ...
    :return:
    """
    return "My id for this room is {}, and it's not apnorton's fault.".format(
        msg._client._br.user_id
    )
...
def unnotify(msg, room_id, se_site):
    ...
    :return: A string
    """
    response = remove_from_notification_list(
        msg.owner.id, msg._client.host, room_id, se_site
    )

    if response:
        return (
            "I will no longer ping you if I report a post on `{site}`, in room `{room}` "
            "on `chat.{domain}`".format(
                site=se_site, room=room_id, domain=msg._client.host
            )
        )
...
RETURN_NAMES = {
    "admin": ["admin", "admins"],
    "blacklist_manager": ["blacklist manager", "blacklist managers"],
}
VALID_ROLES = {
    "admin": "admin",
...
def whois(msg, role):
    ...
    if role not in VALID_ROLES:
        raise CmdException(
            "That is not a user level I can check. "
    ...
    if admins_in_room_list:
        admins_in_room_list.sort(
            key=lambda x: x[2]
        )  # Sort by last message (last seen = x[3])
...
def invite(msg, room_id, roles):
    ...
    return (
        ...
        " (Note that this will not persist after restarts.)".format(
            roles, room_id, msg._client.host
        )
    )
...
def report(msg, args, alias_used="report"):
    ...
    message_url = "https://chat.{0}/transcript/{1}?m={2}".format(
        msg._client.host, msg.room.id, msg.id
    )
    ...
    output = report_posts(
        urls, msg.owner, msg.room.name, message_url, alias_used, custom_reason
    )
...
def allspam(msg, url):
    ...
    if user[1] == "stackexchange.com":
        ...
        # Fetch sites
        request_url = "https://api.stackexchange.com/2.2/users/{}/associated".format(
            user[0]
        )
        params = {"filter": "!6Pbp)--cWmv(1", "key": api_key}
        ...
        for site in res["items"]:
            if site["question_count"] > 0 or site["answer_count"] > 0:
                user_sites.append(
                    (site["user_id"], get_api_sitename_from_url(site["site_url"]))
                )
    else:
    ...
    for u_id, u_site in user_sites:
        ...
        add_blacklisted_user(
            user,
            message_url,
            sorted(posts, key=lambda x: x["score"])[0]["owner"]["link"],
        )
        ...
        for post in posts:
            ...
            (
                *discard,
                post_data.site,
                post_data.post_type,
            ) = fetch_post_id_and_site_from_url(url_to_shortlink(post["link"]))
            ...
            if post_data.post_type == "answer":
                ...
                if GlobalVars.api_backoff_time > time.time():
                    time.sleep(GlobalVars.api_backoff_time - time.time() + 2)
                # Fetch posts
                req_url = "https://api.stackexchange.com/2.2/answers/{}".format(
                    post["post_id"]
                )
                params = {"filter": "!*Jxb9s5EOrE51WK*", "key": api_key, "site": u_site}
                answer_res = requests.get(req_url, params=params).json()
                if "backoff" in answer_res:
                    if (
                        GlobalVars.api_backoff_time
                        < time.time() + answer_res["backoff"]
                    ):
                        GlobalVars.api_backoff_time = (
                            time.time() + answer_res["backoff"]
                        )
    ...
    for index, post in enumerate(user_posts, start=1):
        ...
        handle_spam(
            post=Post(api_response=post.as_dict),
...
def report_posts(
    ...
    reported_by_name = (
        reported_by_owner if type(reported_by_owner) is str else reported_by_owner.name
    )
    ...
    if reported_in is None:
        ...
        reported_from = " by user *{}* in room *{}*".format(
            reported_by_name, reported_in
        )
    ...
    for index, url in enumerate(urls, start=1):
        ...
        if post_data is None:
            output.append(
                "Post {}: That does not look like a valid post URL.".format(index)
            )
            continue

        if post_data is False:
            output.append(
                "Post {}: Could not find data for this post in the API. "
        ...
        if (
            ...
            if GlobalVars.metasmoke_key is not None:
                ...
                output.append(
                    "Post {}: Already recently reported [ [MS]({}) ]".format(
                        index, ms_link
                    )
                )
                continue
        ...
        if fetch_post_id_and_site_from_url(url)[2] == "answer":
            parent_data = api_get_post(
                "https://{}/q/{}".format(post.post_site, post_data.question_id)
            )
        ...
        if operation in {
            "report",
            "report-force",
        }:  # Force blacklist user even if !!/report falls back to scan
            if user is not None:
        ...
        # scan_spam == False and "scan"
            ...
            if scan_why:
                output.append(
                    "Post {}: Looks like spam but not reported: {}".format(
                        index, scan_why.capitalize()
                    )
                )
            else:
...
def dump_data():
    try:
        ...
        s = "{}, {}, {}\n{}".format(
            metadata["time"], metadata["location"], metadata["rev"], s
        )
...
def load_data(msg_id):
    ...
    if (
        msg.owner.id != 120914
    ):  # TODO: implement an is_self() in chatcommunicate, don't use magic numbers
...
@command(
    ...
    reply=True,
    privileged=True,
    aliases=[alias + "-force" for alias in DELETE_ALIASES],
)
...
def false(feedback, msg, comment, alias_used="false"):
    ...
    if user is None:
        if feedback_type.blacklist:
            ...
            raise CmdException(
                result + ", but could not get user from URL: `{0!r}`".format(owner_url)
            )
    else:
        if feedback_type.blacklist:
...
def naa(feedback, msg, comment, alias_used="naa"):
    ...
    if post_type != "answer":
        raise CmdException(
            "That report was a question; questions cannot be marked as NAAs."
        )
    ...
    return (
        "Recorded answer as an NAA in metasmoke."
        if not feedback_type.always_silent
        else ""
    )
...
def true(feedback, msg, comment, alias_used="true"):
    ...
    cant_get_user_command_exception_text = (
        result + ", but could not get user from URL: `{0!r}`".format(owner_url)
    )
    try:
        user = get_user_from_url(owner_url)
    ...
    if not post_data:
        ...
        *post, _ = fetch_post_id_and_site_from_url(post_data[0])
        ...
        if why_info:
            ...
            raise CmdException(
                "I don't have the `why` data for that post (anymore?). "
...
def autoflagged(msg):
    ...
    if is_autoflagged:
        return "That post was automatically flagged, using flags from: {}.".format(
            ", ".join(names)
        )
    else:
